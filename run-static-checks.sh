...
echo "Host worker directory contents:"
ls -la ./worker

docker run --rm -v "$(pwd)/worker:/src" -w /src mcr.microsoft.com/dotnet/sdk:8.0 bash -lc "pwd && ls -la && dotnet format ./Worker.csproj --verify-no-changes --severity error"
